# Magos Biologis Dossier — station-ix
...
- **Body kind:** `planet`
- **Planet type (Administratum):** `fortress_world`
- **Local notes:** —
...
- Topology: Distributed sensor net hub.
...
- `station_ix_a` — class `station_hab`, richness `barren`, medium `industrial_void`, overlay=True
- `station_ix_b` — class `dock_hull`, richness `sparse`, medium `industrial_void`, overlay=True

## Trophic webs (per biome)

### station_ix_a

- **microbiota** — filter_biofilm | Origin: `exotic` / `deliberate_transplant` | analogue: `filter_biofilm`
